...
- This can be modelled offline (e.g. using LUCY, LQF) and then values supplied. The offline models can be used to derive parameters for models within SUEWS  (e.g. Ward and Grimmond 2017).
...
For each material type there are different characteristics needed for the radiative, conductive and water behaviour.
 ...
 .. list-table:: Material related parameters
   ...
   * - Storage Heat 
     - - there  are two models OHM and ESTM to choose from, parameters needed vary with which is chosen
...
.. list-table:: Profile types 
   ...
   * - Snow clearing
     - when does this occur? what is the prioriy order for a region
...
-----------------
 ...
 - Does the water from the roof all go to drains  or does some go on to grass?
